module Minimax
  ...
  def score(board)
    ...
    when human.marker then -100
    when computer.marker then 100
    else 0
    end
  end
end
...
module Displayable
  ...
  def display_computer_choice(computer_choice)
    if computer_choice == human.marker
...
class TTTGame
  ...
  attr_accessor :current_marker, :first_mover
  ...
  def player_chooses_first_mover
    if yes?("To go 1st type --> y \nTo go 2nd type --> n")
      change_first_to_move(human.marker)
    else
      change_first_to_move(computer.marker)
    end
  end

  def computer_chooses_first_mover
    display_computer_choosing
    computer_choice = [COMPUTER_MARKER, human.marker].sample
  ...
  def reset
    board.reset
    self.current_marker = first_mover
    clear
  end
