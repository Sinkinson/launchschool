...
# Make the necessary changes to our code so that the various lengths can be properly sorted and line 62 produces
# the expected output.
...
# The sort method uses the <=> method to compare elements. We therefore need to implement a <=> method
# in our class. If we also include Comparable within our class we also don't have to define all the
# other methods relating to comparison: ==, <, <=, >, >=
# This is because it implements all those methods based off the <=> method
