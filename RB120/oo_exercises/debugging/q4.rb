...
# serial number. However, when we are testing our program, an exception is raised. Fix the code so that
# the program works as expected without error.
...
# The getter method for serial_number is private so therefore can not be invoked with an explicit caller.
# And it is invoked in the == method with an explicit caller. We therefore can just make the serial_number
# getter method protected instead.
...
# method is private so therefore cannot be called using an explicit self.
# This can be corrected by making the method protected instead
