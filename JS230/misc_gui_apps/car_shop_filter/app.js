...
class App {
  constructor() {
    ...
    this.makeFilter = document.querySelector('#make-select');
    this.makesAndModels = this.combineMakesAndModels();
    this.bindEvents();
  }

  bindEvents() {
    this.filterBtn.addEventListener('click', this.filterCars.bind(this));
    this.makeFilter.addEventListener('change', this.filterModels.bind(this));
  }

  filterModels() {
    let modelFilter = document.querySelector('#model-select');
    let modelFilterOptions = Array.from(modelFilter.querySelectorAll('option'));
    
    if (this.makeFilter.value === 'All') {
      modelFilterOptions.forEach(option => option.hidden = false);
    } else {
      let filtered = this.makesAndModels[this.makeFilter.value];
      modelFilterOptions.forEach(option => {
        filtered.includes(option.value) ? option.hidden = false : option.hidden = true;
      });
    }
  }

  combineMakesAndModels() {
    let makesAndModels = {};
    cars.forEach(car => {
      makesAndModels[car.make] = makesAndModels[car.make] || [];
      if (!(makesAndModels[car.make].includes(car.model))) {
        makesAndModels[car.make].push(car.model);  
      }
    });
