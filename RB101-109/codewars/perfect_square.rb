# You might know some pretty large perfect squares. But what about the NEXT one?
# Complete the findNextSquare method that finds the next integral perfect square 
# after the one passed as a parameter. Recall that an integral perfect square is 
# an integer n such that sqrt(n) is also an integer. If the parameter is itself 
# not a perfect square then -1 should be returned. You may assume the parameter 
# is non-negative.

def find_next_square(sq)
  return -1 if Math.sqrt(sq).to_s.split('.').last != '0'
  loop do 
   sq += 1
   break if Math.sqrt(sq).to_s.split('.').last == '0'
  end
  sq
end

p find_next_square(121)
p find_next_square(15241383936)
p find_next_square(319225)
p find_next_square(625)
p find_next_square(155)
p find_next_square(342786627)

# More succinct solution

def find_next_square(sq)
  number = Math.sqrt(sq) + 1
  number % 1 == 0 ? number**2 : -1
end
